...
It is interesting to note that in terms of social robot behavior LfD is mostly used for learning high level behaviours \cite{louie2016learning} \cite{lockerd2004tutelage}. Our work fills this gap, by learning low level control of a social robot from demonstrations. LfD seems to be instead most commonly used for complex manipulation tasks \cite{argall2009survey} where the field is indeed very active. Moreover, deep learning has been used in an end-to-end fashion to perform BC in self-driving cars []. Our work does not perform end to end learning for several reasons. Firstly, people detection modules are typically common to several robot functions such as navigation. Secondly learning end-to-end would require vast amounts of data as well as processing being offloaded to another machine. In contrast we only require a modest amount of data ($\approx 1hr$) and perform all processing on the robot. Finally the RGBD field of view of our robot is quite limited. People detection relies instead on a sensor fusion pipline that uses both the RGBD camera and the laser rangefinders of the robot to enable $360^o$ tracking.  
