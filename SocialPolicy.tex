...
In this paper, we propose \emph{deep behavioural cloning} (DBC), a new LfD method that overcomes these challenges.  \emph{Behavioural cloning} is an approach to LfD in which a control policy is directly inferred using supervised learning, i.e., the demonstrated actions are treated as labels for the corresponding observations, and a mapping from observations to actions is learned.  By leveraging the power of deep learning, DBC makes behavioural cloning practical for social robotics.  In particular, DBC ``paints'' the output of the robot's person detection and localisation system onto a 2D ``image", which is then fed to a convolutional neural network.  In this way, DBC can cope elegantly with varying numbers of people, as well as automatically discover useful features for selecting actions.  In addition, DBC employs \emph{long short-term memory} (LSTM) network layers, allowing it to condition action selection on its history of observations and thus filter out transient perceptual errors.
...
%	\hspace{-5cm}
  ...
  \end{subfigure} 
  %\vspace{-3mm}
  \caption{The two tasks we consider, with arrows showing possible motion of the robot and people.  \sw{This figure is confusing because the arrows mean different things in the two parts, and they are the same except for the arrows so it doesn't really capture the differences.}}
